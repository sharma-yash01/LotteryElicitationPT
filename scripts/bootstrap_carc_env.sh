...
#   export LEPT_ROOT=/scratch1/yashaswi/lept/LotteryElicitationPT
#   export LEPT_VENV=/scratch1/yashaswi/lept/lept-venv
#   bash scripts/bootstrap_carc_env.sh

set -euo pipefail

: "${LEPT_ROOT:?Set LEPT_ROOT to the absolute path of LotteryElicitationPT on CARC}"
...
# ------------------------------------------------------------------ venv
if [[ -d "$LEPT_VENV" ]]; then
    echo ">>> Venv already exists at $LEPT_VENV, reusing."
else
